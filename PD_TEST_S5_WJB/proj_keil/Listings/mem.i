...
static struct mcb  *mcb_head;        
static struct mcb  *mcb_tail;        
...
void     *__malloc(unsigned int size)
{
	...
	for (mcb_ptr = mcb_head; mcb_ptr != mcb_tail; mcb_ptr = mcb_ptr->mcb_next)
	{
		    ...
		    (( free_size = (unsigned)mcb_ptr->mcb_next - (unsigned)mcb_ptr) >= size))
		{
			...
			if ( (free_size - size) >= ((sizeof(struct mcb) + 3) & ~3u))
			{
			    new_mcb = (struct mcb*)((unsigned)mcb_ptr + size);  
			...
			mcb_ptr->mcb_used = 0xDEADBEEF;
			return (void*)((unsigned)mcb_ptr + ((sizeof(struct mcb) + 3) & ~3u));    
		}
	}
...
void      __free(void *mem)
{
	...
	if (((unsigned)(mem) >= (mcb_start + ((sizeof(struct mcb) + 3) & ~3u)) && (unsigned)(mem) <= (mcb_end - 16) && ((unsigned)(mem) & 3) == 0) == 0)
		return;
	free = (struct mcb*)((unsigned)(mem) - ((sizeof(struct mcb) + 3) & ~3u));
	if (free->mcb_used != 0xDEADBEEF)
		return;
