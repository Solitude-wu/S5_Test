...
static void stackDump(uint32_t stack[])
{
    ...
    printf("psr = 0x%x\n", stack[psr]);
#else
	snprintf(buf, 16, "r0  = 0x%x\n", stack[r0]);
	puts(buf);
	snprintf(buf, 16, "r1  = 0x%x\n", stack[r1]);
	puts(buf);
    snprintf(buf, 16, "r2  = 0x%x\n", stack[r2]);
	puts(buf);
    snprintf(buf, 16, "r3  = 0x%x\n", stack[r3]);
	puts(buf);
    snprintf(buf, 16, "r12 = 0x%x\n", stack[r12]);
	puts(buf);
    snprintf(buf, 16, "lr  = 0x%x\n", stack[lr]);
	puts(buf);
    snprintf(buf, 16, "pc  = 0x%x\n", stack[pc]);
	puts(buf);
    snprintf(buf, 16, "psr = 0x%x\n", stack[psr]);
	puts(buf);
#endif
}
...
volatile uint8_t print_onoff = 1;
